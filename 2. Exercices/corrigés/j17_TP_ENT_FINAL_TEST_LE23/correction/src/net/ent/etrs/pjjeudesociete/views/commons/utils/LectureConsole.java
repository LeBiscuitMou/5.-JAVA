package net.ent.etrs.pjjeudesociete.views.commons.utils;
...
public final class LectureConsole {
    ...
    /**
     * Lecture d'un float. Si la valeur saisie n'est pas un float, le message
     * "Choix invalide" est affiché, et la méthode attend une nouvelle saisie.
     *
     * @return le float.
     */
    public static float lectureFloat() {
        float retour = -1;
        boolean boucle = true;
        do {
            boucle = false;
            try {
                String chaine = lectureClavier.nextLine();
                retour = Float.parseFloat(chaine);
            } catch (NumberFormatException e) {
                System.out.println("Choix invalide");
                boucle = true;
            }
     ...
     * @return le float.
     */
    public static float lectureFloat(final String entete) {
        AffichageConsole.afficherMessageSansSautLigne(entete);
        return lectureFloat();
    }
